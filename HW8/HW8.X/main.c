...
int main() {
    ...
    while (1) {
        i++;
        _CP0_SET_COUNT(0);
        ssd1306_drawMessage(0,0,"ABCDEFGHIJKLMNOPQRSTUVWXYZ\0");
        ssd1306_drawMessage(0,10,"abcdefg123456789?!@#{)>;\0");
        sprintf(msg, "%d", i);
        ssd1306_drawMessage(0,20,msg);
        ssd1306_update();
        
        time = _CP0_GET_COUNT();
        fps = 24000000./time;
        sprintf(msg, "fps: %f", fps);
        ssd1306_drawMessage(50,20,msg);
        ssd1306_update();
